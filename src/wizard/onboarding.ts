...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Raven is still in beta. Expect sharp edges.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "If you're not comfortable with basic security and access control, don't run Raven.",
      ...
      "raven security audit --fix",
      "",
      "Must read: https://docs.raven.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Raven onboarding");
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.raven.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
