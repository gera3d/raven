...
function isDisabledByEnv() {
  if (
    isTruthyEnvValue(process.env.RAVEN_DISABLE_BONJOUR) ||
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Raven";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\((?:Raven|OpenClaw)\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.RAVEN_MDNS_HOSTNAME?.trim() ||
